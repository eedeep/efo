...
class Feature(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType,
        default='article',
        limit_choices_to={ 'model': 'article' }) # article for now.
    ...
    def __unicode__(self):
        try:
            if self.content_object.is_published:
                is_published=u'Published'
            else:
                is_published=u'Not Published'
            if self.active:
                return u' %s    (Active & %s)' % (self.content_object, is_published)
            else:
                return u' %s    (Not active / %s)' % (self.content_object, is_published)
        except AttributeError:
            return u' %s    (Missing / Needs re-save)' % (self.content_object)
